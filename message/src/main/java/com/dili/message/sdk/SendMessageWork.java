...
public class SendMessageWork implements Runnable {
	...
	public void run() {
		if (templateType == TemplateType.DELIVERY) {
			//取货
			List<DeliveryParam> sendParam = JSONObject.parseArray(param, DeliveryParam.class);
			messageService.delivery(sendParam);
		} else if (templateType == TemplateType.CAMPAIGN_FAILURE) {
			//团购失败
			List<CampaignFailureParam> sendParam = JSONObject.parseArray(param, CampaignFailureParam.class);
			messageService.campaignFailure(sendParam);
		} else if (templateType == TemplateType.CAMPAIGN_SUCCESS) {
			//团购成功
			List<CampaignSuccessParam> sendParam = JSONObject.parseArray(param, CampaignSuccessParam.class);
			messageService.campaignSuccess(sendParam);
		} else if (templateType == TemplateType.CLOSE_ORDER) {
			//关闭订单
			List<CloseOrderParam> sendParam = JSONObject.parseArray(param, CloseOrderParam.class);
			messageService.closeOrder(sendParam);
		} else if (templateType == TemplateType.DELIVERY_SUCCESS) {
			//取货成功
			List<DeliverySuccessParam> sendParam = JSONObject.parseArray(param, DeliverySuccessParam.class);
			messageService.deliverySuccess(sendParam);
		} else if (templateType == TemplateType.PAY_SUCCESS) {
			//支付成功
			List<OrderPaySuccessParam> sendParam = JSONObject.parseArray(param, OrderPaySuccessParam.class);
			messageService.orderPaySuccess(sendParam);
		} else if (templateType == TemplateType.REFUND) {
			//退款
			List<RefundParam> sendParam = JSONObject.parseArray(param, RefundParam.class);
			messageService.refund(sendParam);
		} else if (templateType == TemplateType.GOODS_WARNING) {
			//商品可用量告警
			List<GoodsWarningParam> sendParam = JSONObject.parseArray(param, GoodsWarningParam.class);
